...
export default function PublicVehicleCard({
  ...
  const calendarHref = isDemo
    ? PUBLIC_ROUTES.calendar
    : `${PUBLIC_ROUTES.calendar}?vehicule=${encodeURIComponent(vehicle.slug)}`;
    ...
    return (
      <Link href={href} className="de-keys-car">
        ...
        <div className="de-keys-car-body">
          ...
          <p className="de-keys-car-dispos">Dates &amp; estimation sur la fiche</p>
        </div>
      </Link>
    );
  }
  ...
  return (
    <article className="de-keys-lot-wrap">
      ...
      <Link href={calendarHref} className="de-keys-lot-dispos">
